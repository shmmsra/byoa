    ...
    if (_windowType == WINDOW_TYPE::POPUP) {
        ...
        [NSEvent addLocalMonitorForEventsMatchingMask:NSEventMaskKeyDown
                                              handler:^NSEvent *(NSEvent *event) {
                                                if (event.window == nsWindow && event.keyCode == 53 && _isWindowVisible &&
                                                    _windowType == WINDOW_TYPE::POPUP) { // Escape key code is 53
                                                    Logger::getInstance().info(
                                                        "WindowWrapper::WindowWrapper: Native escape key pressed, hiding window");
                                                    hide();
...
void WindowWrapper::move() {
    ...
    NSPoint newTopLeftPoint = NSMakePoint(mouseLocation.x,
    ...
    NSPoint bottomLeftPoint = NSMakePoint(newTopLeftPoint.x, newTopLeftPoint.y - currentFrame.size.height);
...
void WindowWrapper::resize(const int &width, const int &height, const bool &animate /* = false*/) {
    ...
    NSRect newFrame = NSMakeRect(currentFrame.origin.x, currentFrame.origin.y - (height - currentFrame.size.height), width, height);
