...
export async function InvokeLLM(baseURL: string, modelName: string, apiKey: string, prompt: string) {
    ...
    try {
        ...
        if (window.saucer?.exposed?.network_fetch) {
            ...
            const responseJson = await window.saucer.exposed.network_fetch(llmURL, JSON.stringify(options));
